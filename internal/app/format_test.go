...
// writeFormatConfig drops a .explorr/format.json into root with the
...
// (e.g. the gofmt entry in ~/.config/explorr/format-defaults.json)
...
func useTestTrustFile(t *testing.T) string {
	...
	t.Setenv("EXPLORR_TRUST_FILE", trustPath)
	t.Setenv("EXPLORR_DEFAULTS_FILE", filepath.Join(dir, "no-such-defaults.json"))
	return trustPath
}
...
func useTestDefaultsFile(t *testing.T, body string) string {
	...
	t.Setenv("EXPLORR_DEFAULTS_FILE", path)
	return path
}
...
// without a .explorr/format.json, save behaves exactly like before
