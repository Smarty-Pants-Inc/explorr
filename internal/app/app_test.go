...
// Quit, moves: 55 -> 57.
//
// Bumped again by Lane B stage 3, which added NINE rows to that same Debug
// group (49 -> 58 items, modalHeight 60 -> 69): Step over / Step into / Step
...
//
// Then "Toggle block comment" landed next to "Toggle line comment", which sits
...
// for actions like Upgrade Explorr and made them appear broken.
...
func TestRunCustomAction_ExecutesAndPostsEvent(t *testing.T) {
	...
	// scribble into the developer's real ~/.local/state/explorr/.
	t.Setenv("XDG_STATE_HOME", t.TempDir())
	dir := t.TempDir()
