...
//
//  1. Load <root>/.explorr/format.json. Missing → done.
...
// install it into the project's .explorr/format.json. Skips
// silently when:
//
...
// .explorr/format.json. Yes merges the entry, auto-trusts the
...
func (a *App) openFormatInstallPrompt(idx int, ext string, argvTemplate []string) {
	...
	a.openConfirm(title, msg, func(app *App) {
		...
		// .explorr/format.json appears in the sidebar without
