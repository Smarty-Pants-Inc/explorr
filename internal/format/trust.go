...
// OK with this project's .explorr/format.json running its
...
// ~/.config/explorr/format-trust.json.
//
...
var trustFileEnv = "EXPLORR_TRUST_FILE"

// DefaultTrustPath returns the canonical trust-file location:
// $XDG_CONFIG_HOME/explorr/format-trust.json, falling back to
// ~/.config/explorr/format-trust.json. Returns "" when neither
// resolves — callers treat that as "no persistent trust available"
// (every save will re-prompt, which is annoying but safe).
//
// Tests can override the location by setting EXPLORR_TRUST_FILE.
func DefaultTrustPath() string {
	if override := os.Getenv(trustFileEnv); override != "" {
		return override
	}
	if xdg := os.Getenv("XDG_CONFIG_HOME"); xdg != "" {
		return filepath.Join(xdg, "explorr", "format-trust.json")
	}
	home, err := os.UserHomeDir()
	if err != nil || home == "" {
		return ""
	}
	return filepath.Join(home, ".config", "explorr", "format-trust.json")
}
