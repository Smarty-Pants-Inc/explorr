...
//
//	$XDG_CONFIG_HOME/explorr/format-defaults.json   (preferred)
//	~/.config/explorr/format-defaults.json          (fallback)
//
...
// .explorr/format.json on consent). That's the only time defaults
...
var defaultsPathEnv = "EXPLORR_DEFAULTS_FILE"

// DefaultsPath returns the canonical defaults-file location:
// $XDG_CONFIG_HOME/explorr/format-defaults.json, falling back to
// ~/.config/explorr/format-defaults.json. Returns "" when neither
// resolves — the caller treats that as "no defaults file possible"
// and skips the install-prompt feature entirely.
//
// Tests can override the path via EXPLORR_DEFAULTS_FILE.
func DefaultsPath() string {
	if override := os.Getenv(defaultsPathEnv); override != "" {
		return override
	}
	if xdg := os.Getenv("XDG_CONFIG_HOME"); xdg != "" {
		return filepath.Join(xdg, "explorr", DefaultsFile)
	}
	home, err := os.UserHomeDir()
	if err != nil || home == "" {
		return ""
	}
	return filepath.Join(home, ".config", "explorr", DefaultsFile)
}
...
// .explorr directory and the file if either is missing. Writes
